...
export const AdminProvider = ({ children }) => {
    ...
    const seedProducts = initialProducts.map(p => ({
        ...p,
        // strip the imported IMAGES references — store url strings
    ...
    // ── Computed stats ──
    const stats = {
        totalRevenue: state.orders.filter(o => o.status !== 'cancelled').reduce((s, o) => s + o.total, 0),
        totalOrders: state.orders.length,
        totalProducts: state.products.length,
        totalCustomers: state.customers.length,
        pendingOrders: state.orders.filter(o => o.status === 'pending').length,
        lowStockProducts: state.products.filter(p => p.stock <= 30),
        recentOrders: state.orders.slice(0, 5),
        topProducts: [...state.products].sort((a, b) => b.reviewCount - a.reviewCount).slice(0, 5),
        ordersByStatus: {
    ...
    const value = {
        ...
        // Review actions
        updateReviewStatus: (id, status) => dispatch({ type: 'UPDATE_REVIEW_STATUS', payload: { id, status } }),
